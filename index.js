 ...
 * @param {Discord.GuildMember} member Le membre à vérifier
 * @returns {boolean} Le membre peut être ignoré
 */

/**
 * @callback IgnoreRoleFunction
 * @param {Discord.Collection<Discord.Snowflake, Discord.Role>} role Le rôle à vérifier
 * @returns {boolean} Le membre ayant le rôle
 */

/**
 * @callback IgnoreGuildFunction
 * @param {Discord.Guild} guild Le serveur à vérifier
 * @returns {boolean} Si le serveur est ignoré
 */
...
/**
 * Quand le membre est avertis
 * @event AntiSpamClient#warnAdd
 * @property {Discord.GuildMember} member Le membre à pris un avertissement
 */

/**
 * Quand le membre est kick
 * @event AntiSpamClient#kickAdd
 * @property {Discord.GuildMember} member Le membre à était kick
 */

/**
 * Quand le membre est mute
...
class AntiSpamClient extends EventEmitter {
	...
	log (message, client) {
		if (this.options.modLogsEnabled) {
			...
			client.channels.cache.filter((channel) => channel.type === 'text').find((channel) => channel.name === this.options.modLogsChannelName)
			if (modLogChannel) {
